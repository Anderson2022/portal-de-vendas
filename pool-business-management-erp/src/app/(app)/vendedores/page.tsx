...
export default async function VendedoresPage({
  ...
  const openBy = (id: string) =>
    quotes.filter(
      (q) =>
        q.salespersonId === id &&
        (q.status === "ORCAMENTO" || q.status === "NEGOCIACAO"),
    ).length;
          ...
          <div key={s.id} className="flex items-center gap-3.5">
            <span
              className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[13px] font-extrabold ${
                i === 0
                  ? "bg-sun-100 text-[#b57708]"
                  : i === 1
                    ? "bg-ink-100/80 text-ink-500"
                    : i === 2
                      ? "bg-[#f3e6dd] text-[#a0664a]"
                      : "bg-ink-100/50 text-ink-300"
              }`}
            >
              {i + 1}º
            </span>
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1.5 flex items-center gap-2.5">
                <Bar
                  value={s.goalPct}
                  tone={s.goalPct >= 100 ? "green" : undefined}
                  className="flex-1"
                />
                <span className="text-[11px] font-bold tabular-nums text-ink-300">
                  {pct(s.goalPct, 0)} da meta
                </span>
              </div>
            </div>
          </div>
  ...
  const metasBlock = (
    <Card className="fade-up overflow-hidden p-6 pb-3">
      <SectionTitle
        ...
        title="Metas e comissões de maio"
      />
                ...
                <td>
                  <div className="flex items-center gap-2.5">
                    <Bar
                      ...
                      className="flex-1"
                    />
                    <span className="w-11 text-right text-[12px] font-extrabold tabular-nums text-ink-700">
                      {pct(s.goalPct, 0)}
                    </span>
                  </div>
                </td>
  ...
  return (
    <div className="mx-auto max-w-[1460px]">
      ...
      {top && f !== "metas" && (
        <div className="fade-up mt-6 grid gap-5 lg:grid-cols-[1.5fr_1fr_1fr]">
            ...
            <div className="relative">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h3 className="mt-1.5 text-[20px] font-extrabold tracking-tight text-ink-950">
                    {top.name}
                  </h3>
                </div>
              </div>
              <div className="mt-5 grid grid-cols-2 gap-4">
                <div>
                  <div className="text-[11.5px] font-extrabold uppercase tracking-[0.08em] text-ink-300">
                    Minha meta
                  </div>
                  <div className="text-[20px] font-extrabold tabular-nums text-ink-950">
                    {brl(n(top.monthlyGoal))}
                  </div>
                </div>
                <div>
                  <div className="text-[11.5px] font-extrabold uppercase tracking-[0.08em] text-ink-300">
                  ...
                  <div className="text-[20px] font-extrabold tabular-nums text-water-700">
                    {brl(top.sold)}
                  </div>
                </div>
              </div>
              <div className="mt-3 flex items-center gap-3">
                <Bar
                  value={top.goalPct}
                  tone={top.goalPct >= 100 ? "green" : undefined}
          ...
          <Card hover className="p-6">
            ...
            <div className="mt-3 text-[13px] font-semibold text-ink-500">
              Comissão acumulada
            </div>
            <div className="text-[26px] font-extrabold tabular-nums text-mint-600">
              {brl(top.commission)}
            </div>
            <div className="mt-1 text-[12px] font-medium text-ink-300">
              Alíquota padrão de {n(top.commissionPct).toLocaleString("pt-BR")}%
              sobre vendas
            </div>
          </Card>
          ...
          <Card hover className="p-6">
            ...
            <div className="flex items-baseline gap-4">
              <div>
                <div className="text-[26px] font-extrabold tabular-nums text-ink-950">
                  {openBy(top.id)}
                </div>
                <div className="text-[11.5px] font-bold text-ink-300">
                  propostas abertas
                </div>
              </div>
              <div>
      ...
      <p className="mt-4 flex items-center gap-2 text-[12.5px] font-medium text-ink-300">
        ...
        Regras de comissão por categoria e por margem estão em Vendas →
        Comissões.
      </p>
    </div>
  );
