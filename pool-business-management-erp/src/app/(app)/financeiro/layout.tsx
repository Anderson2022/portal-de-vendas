...
export default async function Layout({ children }: {
    ...
    if (!canFinance(user.permissions, "view"))
        return <div className="financial-workspace p-8"><FinancialState error title="Acesso não autorizado" description="Seu perfil não possui permissão para visualizar o Financeiro."/></div>;
    return <ModuleShell brand="financeiro" moduleName="Financeiro" items={financialNavigation}><FinancialContext permissions={user.permissions} business={business.tradeName || business.legalName}>{children}</FinancialContext></ModuleShell>;
}
