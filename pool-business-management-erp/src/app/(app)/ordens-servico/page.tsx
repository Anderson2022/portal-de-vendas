...
export default async function OrdensServicoPage({
  ...
  const list =
    f === "hoje"
      ? all.filter((w) => w.scheduledDate === TODAY)
      : f === "instalacao"
        ? all.filter((w) => w.type === "INSTALACAO")
        : f === "manutencao"
          ? all.filter((w) => w.type === "MANUTENCAO" || w.type === "LIMPEZA")
          : all;
  ...
  const running = all.filter(
    (w) => w.status === "EXECUCAO" || w.status === "DESLOCAMENTO",
  ).length;
  const scheduled = all.filter(
    (w) => w.status === "AGENDADA" || w.status === "PENDENTE",
  ).length;
  ...
  return (
    <div className="mx-auto max-w-[1460px]">
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 xl:grid-cols-4">
        ...
        />
        <Stat
          icon={<CirclePlay size={22} strokeWidth={2.2} />}
          label="Em execução"
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
        {list.map((w, i) => {
          ...
          const next =
            stepIdx >= 0 && stepIdx < woFlow.length - 1
              ? woFlow[stepIdx + 1]
              : null;
          return (
            <Card key={w.id} hover className="fade-up flex flex-col p-5">
              ...
              <div className="mt-1 text-[12px] font-bold uppercase tracking-[0.07em] text-ink-300">
                {typeLabel[w.type]}
                {w.poolModel ? ` · ${w.poolModel}` : ""}
              </div>

              <div className="mt-3 flex items-center gap-3">
                <Avatar name={w.customer} size={36} />
                <div className="min-w-0">
                  <div className="truncate text-[13.5px] font-extrabold text-ink-900">
                    {w.customer}
                  </div>
              ...
              <div className="mt-4 flex items-center justify-between border-t border-ink-100/60 pt-3.5">
                <div className="flex items-center gap-3 text-[12px] font-bold text-ink-500">
                  <span className="flex items-center gap-1.5">
                    <CalendarDays size={13} className="text-water-600" />
                    {dateShort(w.scheduledDate)}
                  </span>
                  <span className="flex items-center gap-1.5">
                    <Clock3 size={13} className="text-water-600" />
                    {w.timeSlot ?? "—"}
                  </span>
                </div>
              ...
              {next && (
                <form action={advanceWorkOrder} className="mt-3">
                  ...
                  <Button
                    variant="unstyled"
                    type="submit"
                    className="btn btn-neu w-full !py-2.5 text-[12.5px]"
                  >
      ...
      <p className="mt-5 flex items-center gap-2 text-[12.5px] font-medium text-ink-300">
        ...
        Pendente → Agendada → Em deslocamento → Em execução → Concluída. Pausa e
        cancelamento disponíveis para o gerente.
      </p>
    </div>
  );
}
