...
export default async function InicioPage() {
  ...
  const [vendas, stock, fin, sellers, wos, pools, customers] =
    await Promise.all([
      vendasOverview(),
      stockData(),
      financialData(),
  ...
  const newClients = customers.filter((c) =>
    c.createdAt.startsWith(MONTH),
  ).length;
  const todayWo = wos.filter(
    (w) => w.scheduledDate === TODAY && w.status !== "CANCELADA",
  ).length;
  ...
  const modules: ModuleDef[] = [
    {
      ...
      chip: {
        icon: <Users size={13} />,
        text: `${newClients} novos clientes`,
        tone: "blue",
      },
    },
    {
      href: "/vendas",
      icon: <ShoppingCart size={26} strokeWidth={2} />,
      title: "Vendas",
      desc: "Orçamentos, pedidos e margem real",
      chip: {
        icon: <TrendingUp size={13} />,
        text: `${vendas.openQuotesCount} orçamentos ativos`,
        tone: "green",
      },
    },
    {
      ...
      chip: {
        ...
        text: `${stock.critical} itens críticos`,
        tone: "red",
      },
    },
    {
      href: "/financeiro",
      icon: <HandCoins size={26} strokeWidth={2} />,
      title: "Financeiro",
      desc: "Recebimentos, pagamentos e fluxo de caixa",
      chip: {
        icon: <ChartNoAxesCombined size={13} />,
        text: `${brlCompact(fin.toReceive)} a receber`,
        tone: "green",
      },
    },
    {
      ...
      chip: {
        icon: <Trophy size={13} />,
        text: `${sellers[0]?.name.split(" ")[0] ?? "—"} lidera o mês`,
        tone: "amber",
      },
    },
    {
      ...
      chip: {
        ...
        text: `${todayWo} OS hoje`,
        tone: "blue",
      },
    },
    {
      href: "/piscinas",
      icon: <Waves size={26} strokeWidth={2} />,
      title: "Piscinas",
      desc: "Modelos, volume, equipamentos e garantia",
      chip: {
        icon: <Package size={13} />,
        text: `${pools.length} modelos cadastrados`,
        tone: "blue",
      },
    },
    {
    ...
    <div className="mx-auto max-w-[1680px] px-4 pb-14 pt-7 sm:px-6 lg:px-8">
      ...
      <Card as="section" className="card home-intro fade-up">
        <div>
          <div className="divider-label mb-3">
            {business.tradeName || business.legalName} · {PERIOD_LABEL}
          </div>
          <h1 className="text-2xl font-extrabold tracking-tight sm:text-3xl">
            Bem-vindo, {firstName}
          </h1>
          <p className="mt-2 text-sm text-ink-500">
            Tudo para cuidar do seu negócio em um só lugar.
          </p>
        </div>
      ...
      <Card as="section" className="card mt-7 p-5 sm:p-7">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-extrabold">Seu negócio</h2>
            <p className="mt-1 text-xs text-ink-500">9 módulos · visão geral</p>
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            <ButtonLink
              variant="unstyled"
              href="/vendas"
              className="btn btn-neu !px-4 !py-2"
            >
              Vendas
            </ButtonLink>
            <ButtonLink
        ...
        <div className="inset-soft home-summary">
          <div>
            <strong>{brlCompact(vendas.revenue)}</strong>
            <span>Vendas do mês</span>
          </div>
          <div>
            <strong className="text-mint-600">
              {brlCompact(vendas.profit)}
            </strong>
            <span>Lucro do mês</span>
          </div>
          <div>
            <strong className="text-mint-600">
              {brlCompact(fin.toReceive)}
            </strong>
            <span>A receber</span>
          </div>
          <div>
            <strong>{vendas.openQuotesCount}</strong>
        ...
        <section className="home-modules mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">
          {modules.map((m, i) => (
            <Link key={m.href} href={m.href} className="group">
              <Card
                hover
                className="module-card fade-up flex h-full flex-col items-start gap-4 p-5"
              >
                <div
                  className="icon-tile !h-[44px] !w-[44px] shrink-0 !rounded-[14px] transition-transform duration-300 group-hover:scale-105 group-hover:-rotate-2"
                  style={{ animationDelay: `${i * 50}ms` }}
                >
                  {m.icon}
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="text-[17px] font-extrabold tracking-tight text-ink-950">
                    {m.title}
                  </h3>
                  <p className="mt-2 text-[12px] font-medium leading-snug text-ink-500 [text-wrap:balance]">
                    {m.desc}
                  </p>
                  <span
                    className={`mt-2.5 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ${chipClass[m.chip.tone]}`}
                  >
                    {m.chip.icon}
                    {m.chip.text}
                  </span>
      ...
      <Card className="fade-up mt-7 flex flex-col items-stretch gap-4 p-5 !rounded-[28px] lg:flex-row lg:items-center lg:gap-6 lg:p-6">
        <div className="flex items-center gap-3.5 lg:pl-2">
          ...
          <div>
            <div className="text-[15.5px] font-extrabold text-ink-950">
              Ações rápidas
            </div>
            <div className="text-[12px] font-semibold text-ink-300">
