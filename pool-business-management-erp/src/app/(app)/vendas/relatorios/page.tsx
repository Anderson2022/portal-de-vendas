    ...
    <div className="mx-auto max-w-[1460px]">
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-2">
        <Card className="fade-up p-6 xl:col-span-2">
          ...
          <div className="mt-2 flex items-center gap-5 text-[12px] font-bold text-ink-500">
            <span className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-full bg-water-500" />{" "}
              Faturamento
            </span>
            <span className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-full bg-ink-300" /> Custo das
              vendas
            </span>
          </div>
          <div className="mt-4">
        ...
        <Card className="fade-up p-6">
          <SectionTitle
            icon={<ChartColumnBig size={17} strokeWidth={2.4} />}
            title="Lucro real por mês"
          />
          <div className="mt-4">
            <BarsChart
              id="prof"
              data={r.series.map((s) => ({ a: s.revenue, b: s.profit }))}
              format={brlCompact}
            />
          </div>
          <div className="mt-1 flex items-center gap-5 text-[12px] font-bold text-ink-500">
            ...
            <span className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-full bg-mint-500" /> Lucro
              real
            </span>
          </div>
        </Card>
        ...
        <Card className="fade-up p-6">
          ...
          <div className="mt-5">
            <HBars
              items={r.sellerTotals.map((s) => ({
                label: s.name,
                value: s.total,
                sub: `${s.count} vendas no mês`,
              }))}
              format={brl}
            />
          </div>
        ...
        <Card className="fade-up p-6 xl:col-span-2">
          <SectionTitle
            ...
            title="Produtos e serviços mais vendidos — maio"
          />
          <div className="mt-5">
            <HBars
