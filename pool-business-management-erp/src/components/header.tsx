      ...
      >
        ...
        <div className="hidden flex-1 justify-center md:flex">
          ...
          >
            <Search
              size={16}
              className="text-ink-300 transition group-hover:text-water-600"
            />
            <span className="flex-1 text-[13.5px] font-medium text-ink-300">
              Buscar clientes, pedidos, produtos, OS...
            </span>
            <kbd className="neu rounded-lg px-2.5 py-1 text-[10.5px] font-extrabold text-ink-500">
              ⌘ K
            </kbd>
          </Button>
        </div>
            ...
            {menu === "bell" && (
              ...
              >
                ...
                {alerts.map((a, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-3 rounded-2xl px-3 py-2.5 transition hover:bg-water-50"
                  >
                    <span
                      className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${a.tone === "red" ? "bg-coral-500" : "bg-sun-500"}`}
                    />
                    <div>
                ...
                <Link
                  ...
                  className="mt-1 block rounded-2xl bg-water-50 px-3 py-2.5 text-center text-[12.5px] font-bold text-water-700"
                >
                  Ver estoque crítico
                </Link>
              </Dropdown>
              ...
              >
                ...
                <form action={logout}>
                  <Button
                    variant="unstyled"
                    type="submit"
                    className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-[13px] font-bold text-coral-500 transition hover:bg-coral-100/60"
                  >
                    <LogOut size={15} />
                    Sair
                  </Button>
