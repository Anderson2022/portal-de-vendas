...
export function MediaSection() {
  ...
  return (
    <ProductSection
      ...
      title="Imagem e observações"
      description="A imagem é incluída na ficha exportada. Não é enviada ao cadastro atual."
    >
      <div className="grid gap-5 sm:grid-cols-[180px_1fr]">
        ...
        <div className="space-y-3">
          ...
          {image && (
            <Button onClick={() => setImage("")}>Remover imagem</Button>
          )}
        </div>
        <div className="sm:col-span-2 grid gap-4 sm:grid-cols-2">
          <Field label="Observações internas">
            <Textarea name="internalNotes" rows={4} maxLength={5000} />
          </Field>
          <Field label="Orientações para venda e instalação">
            <Textarea name="salesNotes" rows={4} maxLength={5000} />
          </Field>
        </div>
      </div>
    </ProductSection>
