...
export function RegisteredProductsTable({
  rows,
  productOptions,
}: {
  rows: Row[];
          ...
          <tbody>
            {rows.map((product) => (
              ...
              >
                ...
                <td>
                  {productOptions.categories.find(
                    (category) => category.id === product.values.categoryId,
                  )?.name || "—"}
                </td>
                <td>
                  {productOptions.brands.find(
                    (brand) => brand.id === product.values.brandId,
                  )?.name || "—"}
                </td>
                <td>{product.unit || "—"}</td>
                <td>
                  {String(
                    (product.values as Record<string, string>).productType ||
                      "Produto comum",
                  )}
                </td>
                <td>
                  <Badge
                    tone={product.values.active === "false" ? "slate" : "green"}
                  >
                    {product.values.active === "false" ? "Inativo" : "Ativo"}
                  </Badge>
                </td>
                <td>
