...
export function StockProcessView({
  ...
  kind: "transferencias" | "inventarios";
  s: StockOverviewData;
  productOptions: Pick<ProductFormProps, "warehouses">;
}) {
  const [open, setOpen] = useState(false),
    transfer = kind === "transferencias";
  return (
    <>
      <Card className="p-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            {transfer ? <ArrowLeftRight /> : <ClipboardCheck />}
            <div>
              <h2 className="text-lg font-extrabold">
                {transfer ? "Transferências" : "Inventários"}
              </h2>
              <p className="text-sm text-ink-400">
                {transfer
                  ? "Movimentações entre depósitos."
                  : "Contagem e conferência do estoque."}
              </p>
            </div>
          </div>
          <Button variant="primary" onClick={() => setOpen(true)}>
            {transfer ? "Nova transferência" : "Novo inventário"}
          </Button>
        </div>
      </Card>
      {open && transfer && (
