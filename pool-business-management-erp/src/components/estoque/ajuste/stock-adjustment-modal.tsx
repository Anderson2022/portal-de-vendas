...
} from "./adjustment-product-picker";
import {
  StockAdjustmentHistory,
  type AdjustmentHistoryItem,
} from "./stock-adjustment-history";
...
export function StockAdjustmentModal({
  ...
  const [effect, setEffect] = useState<"ENTRY" | "EXIT">("EXIT"),
  ...
  useEffect(() => {
    setLocations([]);
    setLocationId("");
    if (warehouseId) void listStockLocations(warehouseId).then(setLocations);
  }, [warehouseId]);
  const physical = product?.stock || 0,
    available = physical - (product?.reserved || 0),
    amount = Number(quantity) || 0;
  const after = useMemo(
    () => physical + (effect === "EXIT" ? -amount : amount),
    [physical, effect, amount],
  );
  const availableWarehouses = warehouses.filter(
    (warehouse) => !siteId || String(warehouse.site_id || "") === siteId,
  );
  ...
  function submit(event: React.FormEvent) {
    ...
    if (effect === "EXIT" && amount > available)
      return setError("A quantidade de saída é maior que o saldo disponível.");
  ...
  return (
    ...
    >
      ...
      {tab === "history" ? (
        <StockAdjustmentHistory rows={history} />
      ) : (
        <form className="space-y-5" onSubmit={submit}>
          <div className="grid gap-4 sm:grid-cols-2">
            <AdjustmentProductPicker
              ...
              onChange={(selected) => {
                setProduct(selected);
                if (selected.warehouseId) setWarehouseId(selected.warehouseId);
              }}
            />
            <label className="space-y-1 text-sm font-semibold">
              <span>Unidade / estabelecimento</span>
              <select
            ...
            <label className="space-y-1 text-sm font-semibold">
              <span>Lote</span>
              <Input placeholder="Lote (quando aplicável)" />
            </label>
            <label className="space-y-1 text-sm font-semibold">
              ...
              <select
                ...
                value={effect}
                onChange={(event) =>
                  setEffect(event.target.value as "ENTRY" | "EXIT")
                }
              >
          ...
          <div className="grid gap-4 rounded-2xl bg-white/45 p-4 sm:grid-cols-3">
            <div>
              <span className="text-xs font-bold text-ink-400">
                Saldo físico atual
              </span>
              <strong className="block text-xl">
                {physical} {product?.unit || "UN"}
              </strong>
            </div>
            <div>
              <span className="text-xs font-bold text-ink-400">
            ...
            <div>
              <span className="text-xs font-bold text-ink-400">
                Saldo físico após ajuste
              </span>
              <strong className="block text-xl">
                {after} {product?.unit || "UN"}
              </strong>
            </div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="space-y-1 text-sm font-semibold">
              ...
              <Input
                ...
                placeholder="Ex.: Inventário INV-000123"
              />
            </label>
          ...
          {done && <Notice>Ajuste registrado e saldo atualizado.</Notice>}
