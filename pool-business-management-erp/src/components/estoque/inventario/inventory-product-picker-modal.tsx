...
export function InventoryProductPickerModal({
  ...
  const allVisible =
    visible.length > 0 &&
    visible.every((product) => draft.includes(product.id));
  const confirm = (ids = draft) => {
    onConfirm(ids);
    onClose();
  };
  return (
      ...
      <div className="flex h-full min-h-0 flex-col gap-4">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto rounded-2xl border">
          <table className="w-full text-left text-sm">
            <thead className="sticky top-0 bg-[#eeeeef]">
              <tr>
            ...
            <tbody>
              {visible.map((product) => (
                <tr
                  key={product.id}
                  className="cursor-pointer border-t hover:bg-white/40"
        ...
        <div className="flex shrink-0 items-center justify-between">
          <span className="text-sm text-ink-400">
            {draft.length} produto(s) selecionado(s)
          </span>
