...
UPDATE unidades_medida SET sigla = code WHERE sigla IS NULL;
ALTER TABLE unidades_medida ALTER COLUMN sigla SET NOT NULL;
ALTER TABLE unidades_medida ADD CONSTRAINT ck_unidade_medida_casas_decimais CHECK (casas_decimais BETWEEN 0 AND 6);
ALTER TABLE unidades_medida ADD CONSTRAINT ck_unidade_medida_tipo CHECK (tipo IN ('UNIDADE','PESO','VOLUME','COMPRIMENTO','AREA','EMBALAGEM','OUTRO'));
...
CREATE TABLE produto_unidades_medida (
    ...
    CONSTRAINT ck_produto_unidade_base_fator CHECK (NOT unidade_base OR fator_conversao = 1)
);
CREATE UNIQUE INDEX uk_produto_unidade_base ON produto_unidades_medida (produto_id) WHERE unidade_base AND ativo;
